...
public class PlayerUtils {
    public static boolean isMoving() {
        if ((!Helper.mc.thePlayer.isCollidedHorizontally) && (!Helper.mc.thePlayer.isSneaking())) {
            return ((Helper.mc.thePlayer.movementInput.moveForward != 0.0F || Helper.mc.thePlayer.movementInput.moveStrafe != 0.0F));
        }
        return false;
    }
    ...
    public static boolean isOnGround(double height) {
        if (!Helper.mc.theWorld.getCollidingBoundingBoxes(Helper.mc.thePlayer,Helper.mc.thePlayer.getEntityBoundingBox().offset(0.0D, -height, 0.0D)).isEmpty()) {
            return true;
        } else {
            return false;
        }
